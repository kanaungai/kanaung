    ...
    <section id="how-it-works" className="py-24 md:py-32">
      <div className="max-w-[1200px] mx-auto px-8">
        ...
        <motion.div
          ...
          className="relative rounded-2xl border border-foreground/[0.07] overflow-hidden"
          style={{
        ...
        >
          ...
          <div className="grid md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-foreground/[0.07]">
            {STEPS.map((step, i) => (
              <motion.div
                key={step.number}
              ...
              >
                {/* Step number */}
                <span className="font-sora text-[11px] font-bold tracking-[0.12em] uppercase text-muted-foreground/50 mb-8">
                  {step.number}
                </span>

                {/* Connector dot — desktop only */}
                {i < STEPS.length - 1 && (
                  <div className="hidden md:block absolute top-[52px] right-0 translate-x-1/2 w-2 h-2 rounded-full border-2 border-foreground/15 bg-background z-10" />
                )}

                {/* Title */}
                <h3 className="font-sora text-[18px] font-semibold tracking-[-0.02em] text-foreground leading-snug mb-4">
                  {step.title}
                </h3>

                {/* Description */}
                <p className="text-[14px] text-muted-foreground leading-[1.8] font-inter flex-1">
                  {step.description}
                </p>

                {/* Footer label */}
                <div className="mt-8 pt-6 border-t border-foreground/[0.06]">
                  <span className="text-[11px] font-semibold tracking-[0.08em] uppercase text-foreground/35">
                    {step.label}
                  </span>
                </div>
              </motion.div>
